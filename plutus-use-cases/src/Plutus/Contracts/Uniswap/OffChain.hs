...
funds = do
    ...
    return $ mconcat [getTxOutValue o | o <- os]
...
getUniswapDatum o =
  case o of
      ...
      ScriptChainIndexTxOut { scOutDatum } -> do
        (Datum e) <- either getDatum pure scOutDatum
...
findUniswapInstance us c f = do
    ...
    utxos <- utxosAt addr
    go  [x | x@(_, o) <- Map.toList utxos, isUnity (getTxOutValue o) c]
  where
